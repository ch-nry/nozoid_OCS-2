...
inline void MIDI_in() { // the main loop is faster than midi message, no need to get midi data in interuption
  ...
  if (USART3->US_CSR  & (1<<0)) { // RXRDY
    ...
    if ((midi_data & 0xF0) == 0xF0) { // real time messages
      led1_time = 10; // very small blink

      if (midi_data == 0xF8) { // real time clock
        MIDI_QN++;
        if( MIDI_QN == 24) { // on as eu 24 Quarter Note, ca fait 1 tick : notre untité de temps
          MIDI_QN = 0;
          MIDI_LFO_FQ = 0xFFFFFFFF / LFO3_MIDI_count;
          LFO3_MIDI_count = 0;
        }
      }
    }
